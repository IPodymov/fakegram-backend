...
- 🧪 Протестировать API запросы прямо в браузере  
- 📝 Увидеть структуру запросов и ответов
- 🔐 Авторизоваться с помощью JWT токена (Bearer auth или Cookie)

#### Как использовать Swagger:
1. Откройте `http://localhost:3000/api` в браузере
2. Для авторизации нажмите кнопку "Authorize" вверху справа
3. Вставьте JWT токен в поле "JWT-auth" (получите его через `/auth/login`)
